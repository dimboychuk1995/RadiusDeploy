...
import json
from loads import get_or_create_partner
...
def import_super_dispatch_orders():
    try:
        ...
        for order in filtered_orders:
            ...
            # 🆕 Получаем или создаём брокера
            partner_type = "broker"
            partner_name = order.get("customer", {}).get("name", "").strip()
            partner_email = order.get("customer", {}).get("contact", {}).get("email", "")
            partner_phone = order.get("customer", {}).get("contact", {}).get("phone", "")
